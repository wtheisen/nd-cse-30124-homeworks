...
Final teaching dataset for CSE 30124 HW02, following the mansion investigation in HW01. These are synthetic instructional measurements, not real forensic evidence. Quinn's statement rows are carried over from HW01; the filling and weighing records are fictional additions.
...
All splits are public and downloaded together. The assignment requires students to leave reserved outcomes uninspected until model selection is complete, and never use the scene outcome to tune a model. No precomputed aggregate targets or solution notebooks are included in this directory.
